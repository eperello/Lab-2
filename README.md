...
**W(n)=aW(n/b)+1 --> O(nlogn). W(n)=aW(n/b)+logn --> O(2nlogn). W(n)=aW(n/b)+n --> O(n^2logn)**
...
**If c<log_b a then the tree is leaf dominated. If c>log_b a then the tree is root dominated. If they are equal the tree is balanced.**
